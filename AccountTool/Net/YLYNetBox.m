...
- (void)sendRequestWithAddress:(NSString *)address paramDict:(NSDictionary *)paramDict tag:(NSString *)urlTag name:(NSString *)urlName {
    //补全地址
    NSString *URLString = [NSString stringWithFormat:@"%@%@", BaseURLString, address];

    //检查自定义名称
    self.requestName = @"未命名的name";
    if ([NSString safeString:urlName].length != 0) {
    ...
    if ([NSString safeString:urlTag].length != 0) {
...
#pragma mark - --------- 处理请求回调 ---------
// 请求中
- (void)process:(NSProgress *)process {
    if (self.requestProcessBlock) {
        self.requestProcessBlock(process);
    }
}
//请求成功
